    ...
    private void pwdKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_pwdKeyPressed
       if(evt.getKeyCode()==KeyEvent.VK_ENTER)
        ...
        if(pwd.getText().equals(""))
        {
            ...
            String sql = "SELECT `level`, `pass` FROM `log` WHERE `level` = ? AND `pass` = ?";
            ...
            if(rs.next())
            {
               ...
               // ms.PrincipalLog(false);
                mainPanel.setVisible(true);
                long time = System.currentTimeMillis();
                java.sql.Timestamp timestamp = new java.sql.Timestamp(time);

                String rec = "UPDATE `log` SET `timestamp`= '"+timestamp+"' WHERE `level` = '"+username.getText().toUpperCase()+"'";
                stm = conn.createStatement();
                stm.executeUpdate(rec);
            }else{
